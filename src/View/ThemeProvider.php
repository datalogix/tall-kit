...
class ThemeProvider
{
    ...
    {
        $this->items = $this->items->mergeRecursive($attrs);
    ...
    public function __get($key)
    {
        ...
        if (key($attributes) === 'class' && is_array(current($attributes)) && is_int(key(current($attributes)))) {
            $attributes = ['class' => implode(' ', current($attributes))];
        }
